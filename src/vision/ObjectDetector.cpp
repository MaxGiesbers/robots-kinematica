#include "robots-kinematica/vision/ObjectDetector.hpp"


ObjectDetector::ObjectDetector(cv::Mat aFrame, cv::Mat aFilteredFrame) : frame(aFrame), 
                                filteredFrame(aFilteredFrame)
{
}
...
void ObjectDetector::setText(std::shared_ptr<ColorObject>& colorObject)
{
    ...
    putText(frame, "x-pos: " + std::to_string((int)colorObject->getCenter_x_pos()), 
        cv::Point((int)colorObject->getCenter_x_pos(),((int)colorObject->getCenter_y_pos()- 70)),
                1, 1,cv:: Scalar(255,255,255), 1,1);
    putText(frame, "y-pos: " + std::to_string((int)colorObject->getCenter_y_pos()), 
        cv::Point((int)colorObject->getCenter_x_pos(),((int)colorObject->getCenter_y_pos() - 90)), 
            1, 1,cv:: Scalar(255,255,255), 1,1);
            
    putText(frame, "Oppervlakte: " + std::to_string((int)colorObject->getArea()), 
        cv::Point((int)colorObject->getCenter_x_pos(),((int)colorObject->getCenter_y_pos() - 120)), 
            1, 1,cv:: Scalar(255,255,255), 1,1);
    putText(frame, colorObject->getFigure() + " " + colorObject->getColor() , 
        cv::Point((int)colorObject->getCenter_x_pos(), ((int)colorObject->getCenter_y_pos() - 140)), 
            1, 1,cv:: Scalar(255,255,255), 1,1);
...
bool ObjectDetector::checkSquareAndRectangle(std::shared_ptr<ColorObject>& colorObject, std::vector <cv::Point>& approx)
{
    ...
    if(sideUpper > deviationSquare && sideDown > deviationSquare && sideLeft > deviationSquare && sideRight > deviationSquare )
    {
        if(std::fabs(sideUpper - sideDown) <= deviationSquare && std::fabs(sideLeft - sideRight) <= deviationSquare )
        {
            ...
            else if (std::fabs(sideUpper - sideLeft) > deviationRectangle && colorObject->getInputFigure().compare("rechthoek")== 0 )
            {
...
void ObjectDetector::findShape(std::shared_ptr<ColorObject> &colorObject)
{
    ...
    {
        std::cout << colorObject->getInputFigure() << " " << colorObject->getColor()
            << " is niet gevonden. " << std::endl;
    }
}
...
void ObjectDetector::filterColor(std::shared_ptr<ColorObject> &colorObject)
{
    ...
    inRange(frameHSV, cv::Scalar(colorObject->colorRange.iLowH, colorObject->colorRange.iLowS, 
        colorObject->colorRange.iLowV),cv::Scalar(colorObject->colorRange.iHighH, 
            colorObject->colorRange.iHighS, colorObject->colorRange.iHighV), colorMask);
